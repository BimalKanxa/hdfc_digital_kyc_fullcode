...
export default function UploadSignaturePage() {
  ...
  const [stage, setStage] = useState("SIGNATURE_UPLOAD");

  useEffect(() => {
    // optional: fetch KYC to get currentStage
    async function fetchKyc() {
      try {
        if (!kycId) return;
        const res = await axios.get(`${API}/api/admin/kycs/${kycId}`); // public kyc endpoint could be used if exists
        // setStage(res.data.data.stage || "SIGNATURE_UPLOAD");
      } catch (e) { /* ignore */ }
    }
    fetchKyc();
  }, []);
  ...
  const handleUpload = async () => {
    if (!file) { setMsg("Please choose signature image"); return; }
    setLoading(true);
    setMsg("");
    ...
    try {
      ...
      const res = await axios.post(`${API}/api/upload-signature`, fd, {
        headers: {
      ...
      setMsg("Signature uploaded. Redirecting...");
      // Save stage change if needed
      localStorage.setItem("kycStage", res.data.nextStage);
      setTimeout(() => { window.location.href = "/upload-passport-photo"; }, 1200);
